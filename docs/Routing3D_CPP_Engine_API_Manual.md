...
작성일: 2026-06-15  
갱신일시: 2026-06-19 12:22:02 KST  
소스수정 버전: 2026.06.19-fix5  
소스수정 일시: 2026-06-19 12:22:02 KST  
소스수정 내용: CBS-lite 확장 제한/진단값 보존, Octree visited/leaf size-query, Viewer sampled blocked preview, OpenVDB sampled iterator  
최근 업데이트: 2026-06-19, 우선 보완 5건 코드 반영 및 API 문서 갱신  
범위: C++ 코어 엔진, 점유맵 백엔드, A* 라우팅, 옥트리 점프 라우팅, 다중 배관 처리, CBS-lite 협상 라우팅, scene.json JSON 저장 포맷, C ABI, C# managed wrapper/tutorial, CLI, 선택 기능(OpenVDB/FCL/Python binding)
...
| 단일 경로 | `astar.hpp`, `cost.hpp` | weighted A* 탐색 및 비용 계산 | `AStarResult` |
| 다중 경로 | `multi_route.hpp`, C API 내부 orchestration | 작업 정렬, 순차 라우팅, 배관 점유 반영 | `MultiRouteResult`, `SceneResult` |
| 대형 격자 | `corridor.hpp`, `ImplicitOccupancy`, `OctreeOccupancy`, `VdbOccupancy` | coarse/fine corridor, sparse 점유, adaptive voxel 처리 | 대형 격자 경로, 옥트리 leaf, 점유 셀 열거 |
...
| 단일 경로탐색 | `include/routing3d/astar.hpp` | 균일 비용 A*, weighted A* |
| 다중 라우팅 | `include/routing3d/multi_route.hpp`, `capi/routing3d_capi.cpp` | 순차 라우팅, 배관 점유 반영, rip-up, CBS-lite |
...
| min straight | `r3d_set_min_straight()` | 코너 전후 최소 직선 길이 확보 |
...
### 5.8 Hierarchical corridor routing

파일: `include/routing3d/corridor.hpp`, C API 내부 `route_hier()`

알고리즘:

1. fine 격자 좌표를 coarse 격자로 축소
2. coarse 점유맵에서 guide path 탐색
3. coarse path 주변을 radius만큼 팽창해 corridor 생성
4. fine A*는 corridor 내부에서만 탐색

용도:

- 초대형 평면/미세 격자에서 전체 공간 탐색을 피한다.
...
7. 필요 시 `r3d_set_per_task_radius(1)`, `r3d_set_pipe_gap(60.0)`, `r3d_set_min_straight(2.0)`
...
4. leaf 개수가 버퍼보다 클 수 있으므로, 현재 API에서는 충분히 큰 버퍼를 사용하고 `out_count == maxCount`이면 잘림 가능성을 표시한다.
5. 다중 배관 최종 배치는 현재 `r3d_route_multi_progress()`, `r3d_route_ripup()`, `r3d_route_corridor_multi()`를 우선 사용한다.
...
| P1 | C API와 core API 기능 차이 | 제품 기능(per-task radius, gap, CBS-lite, min-straight, hierarchical fallback)이 대부분 C API 구현 내부에 있고 core header API와 분산되어 있다 | **부분 완료:** 런타임 옵션을 C API 구조체로 노출. 후속으로 core service layer 분리 필요 |
...
| P1 | 옥트리 다중 라우팅 미통합 | `r3d_route_task_octree()`는 단일 task 중심이며 multi/rip-up/CBS-lite orchestration에는 아직 직접 연결되어 있지 않다 | Octree backend를 route_multi backend 선택지로 승격하고 mark_pipe/rebuild 정책을 명시 |
...
| P2 | 환경변수 의존 | `R3D_MAX_EXP`, `R3D_FALLBACK_EXP`, `R3D_RIPUP` 등 운영 옵션이 env에 의존 | C API setter 및 scene/runtime option 덤프 지원 |
