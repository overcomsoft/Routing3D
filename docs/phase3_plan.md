...
- ✅ **Step 3.7 FCL 정밀 충돌(FclScene)** (`fcl_scene.hpp/.cpp`): 장애물 AABB 를 FCL dynamic-AABB-tree broadphase 에 등록, 50mm 복셀을 넘는 정밀 기하 질의 — `collides_point/sphere`, `segment_clear`(반경 r 캡슐=파이프), `path_clear`(폴리라인). FCL/Eigen 은 pimpl 로 헤더 미노출. **FCL 0.7.0** vcpkg 빌드(~2.6분, +eigen3/ccd/octomap). CMake `-DUSE_FCL=ON` → `routing3d_fcl` 별도 lib. `test_fcl.exe`: FCL 점=해석AABB=점유맵 일치, 구/선분 이격(거리 vs 반경), **틈(폭200) 가는파이프(r50) 통과·굵은파이프(r150) 충돌**(sub-voxel 정밀). **ctest 7/7 통과**(golden/scene_io/occupancy/corridor/vdb/fcl/bindings).

> **빌드 환경 갱신**: vcpkg = `D:/vcpkg`(bootstrap 완료, OpenVDB 12.0.1 + FCL 0.7.0 설치). 전체 옵션 빌드: `cmake -S cpp -B cpp/build -G "Visual Studio 17 2022" -A x64 -DCMAKE_TOOLCHAIN_FILE=D:/vcpkg/scripts/buildsystems/vcpkg.cmake -DVCPKG_TARGET_TRIPLET=x64-windows -DUSE_OPENVDB=ON -DUSE_FCL=ON -DBUILD_PYTHON_BINDINGS=ON -Dpybind11_DIR=... -DROUTING3D_TEST_PYTHON=...`.

> **Step 3.6·3.7 완료**. **다음 후보** = 3.8 rip-up·CBS(성공률↑) · 3.11 벤치/최적화(corridor 튜닝·클리어런스 로컬화·라우팅 시 FCL 정밀검증 통합) · 3.12 회귀 리포트.
