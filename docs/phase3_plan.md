...
> **다음 즉시 실행 항목** = Step 3.6 (part 3) **엔진 백엔드 무관화**(astar/cost/multi_route 를 점유백엔드 템플릿화 → Sparse/Vdb 로 라우팅 + 골든 재현 교차검증) → **계층 corridor**(coarse 경로로 fine 탐색영역 제한, 단일 배관 <1초). 이후 FCL(3.7)/rip-up(3.8)/벤치(3.11)/리포트(3.12).
