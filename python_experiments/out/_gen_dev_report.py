...
def ch_intro():
    return [
        ...
        ("b", [
            ...
            "6. 기존설계 패턴 학습(pgvector) — 동기/데이터(6.1) · 학습 파이프라인(6.2) · "
            "추론·적용 L2a/전처리/L2b(6.3) · 실측(6.4)",
            "7. 전체 결과 요약 및 향후 과제",
        ]),
        ("pb", None),
    ]
...
def ch_pattern():
    return [
        ("h1", "6. 기존설계 패턴 학습 (pgvector)"),
        ("p", "사람이 설계한 기존배관(TB_ROUTE_PATH)의 형상을 학습해 자동라우팅에 활용한다. 핵심 통찰은 "
              "경로의 품질·실패가 거의 양 끝 '스텁'(장비 출발부 / 덕트·레터럴 진입부)에서 결정된다는 것 — "
              "중간 구간은 대개 자유공간 직선이다. 따라서 양 끝 스텁의 형상을 (장비·유틸리티)·(덕트·유틸리티) "
              "키로 학습해 그 형상을 자동라우팅의 출발/종단 처리에 반영한다. DB 에 설치된 pgvector 를 스텁 "
              "표본의 벡터 저장·검색에 사용한다. 본 기능은 엔진 골든 결과를 바꾸지 않는다(회랑은 w_corridor>0 "
              "일 때만 동작, 기본 경로는 바이트 불변)."),

        ("h2", "6.1 동기 · 데이터"),
        ("p", "덕트 상부 PoC 로 가야 할 배관이 덕트를 관통하던 문제를 기하 규칙(설비/덕트 항상 솔리드 + PoC "
              "표면 투영 + 가중 A*)으로 먼저 해결했는데, 이 규칙들은 사실 '사람이 보면 당연한' 출발/종단 "
              "형상을 손으로 코딩한 고정 규칙이다. 기존 설계배관에는 그런 형상이 수천 개 들어 있으므로, "
              "이를 학습해 일반화하면 고정 규칙이 놓치는 코너 케이스를 메우고 결과를 사람 설계에 수렴시킬 수 있다."),
        ("table", (
            ["소스", "내용", "학습에서의 역할"],
            [
                ["TB_ROUTE_PATH (+SEGMENTS/DETAIL)", "기존 설계배관 폴리라인 + 양 끝 PoC + 유틸/그룹/호칭경",
                 "지도학습 라벨(정답 경로). 양 끝 스텁 추출 원천."],
                ["TB_BIM_EQUIPMENT (+POC_LIST)", "장비 AABB + PoC",
                 "출발 스텁의 로컬 프레임(앵커 AABB) + 키(장비·유틸)."],
                ["TB_DUCT_LATERAL", "덕트·레터럴 AABB + UTILITY",
                 "종단 스텁의 로컬 프레임 + 키(덕트·유틸)."],
                ["route_stub_pattern (+뷰 route_stub_template)", "학습 스텁 표본 저장소(pgvector, 우리가 생성)",
                 "feat vector(24)·dir_unit vector(3)·HNSW + 키별 집계(대표 면/방향/rise)."],
            ],
        )),

        ("h2", "6.2 학습 파이프라인 (L1)"),
        ("p", "한 기존배관에서 양 끝 스텁을 추출 → 앵커(장비/덕트) AABB 로컬 프레임으로 정규화 → 특징벡터로 "
              "표현 → pgvector 저장소에 적재한다(Python routing3d_py.pattern_learn). 정규화는 위치·크기가 달라도 "
              "같은 패턴이 한곳에 모이게 한다."),
        ("b", [
            "스텁 추출: PoC 끝에서 누적 길이(≤4m)/꺾임(≤3) 한도까지 폴리라인을 잘라 방향 시퀀스로 정규화.",
            "정규화 특징(24차원): [진출/진입 면 one-hot 6][1차 방향 6][2차 방향 6][앵커 내 PoC 상대좌표 3]"
            "[시작→종단 단위벡터 3]. one-hot/상대좌표/단위벡터라 성분 스케일이 균형 → 그룹 내 L2/코사인이 유의미.",
            "적재: psycopg2 가 vector 를 모르므로 '[a,b,c]' 텍스트 리터럴로 바인딩. 키별 대표값은 SQL 집계뷰"
            "(mode(면/방향)·median(rise))로 즉시 조회.",
            "검증: project6 → 405 표본 / 38 키. 학습된 대표 면이 도메인 상식과 일치 — 출발(장비)=-z(아래로), "
            "종단(덕트)=+z(상부로). 즉 기존 기하 규칙이 데이터로 입증됨. pytest 9/9.",
        ]),
        ("code",
         "# 학습 + 검수 리포트\n"
        ...
        ("h2", "6.3 추론 · 적용 (L2a · 전처리 · L2b)"),
        ("p", "C# 뷰어가 라우팅 직전, 작업의 (장비·유틸)·(덕트·유틸) 키로 학습 결과를 조회해 세 가지로 적용한다. "
              "모두 미적재/키 미스 시 기존 기하 규칙으로 자동 폴백한다(무해, 회귀 0)."),
        ("table", (
            ["적용", "방식", "엔진 영향"],
            [
                ["L2a 학습면 PoC 투영", "PoC 표면 투영을 '최근접 면' 대신 '학습된 면'으로(장비=아래·덕트=상부). "
                 "PatternStore + LiftPocToSurface(preferFace).", "없음(C#만)"],
                ["접근불가 PoC 전처리", "Lift 후에도 막힌(파묻힌) PoC 를 학습면 행진+반경확장으로 최근접 자유셀로 "
                 "스냅(SnapPocToFreeCell).", "없음(C#만)"],
                ["L2b 기존설계 회랑(옵트인)", "매칭되는 기존 설계배관 폴리라인을 회랑 셀로 주입(신규 C ABI "
                 "r3d_set_corridor_cells) + w_corridor 로 회랑 밖 가산 → 새 경로가 사람 설계를 부드럽게 추종.",
                 "corridor 키 long long 확장(골든 불변)"],
            ],
        )),

        ("h2", "6.4 실측 (project6, cell=100, 208 작업)"),
        ("table", (
            ["구성", "성공", "시간", "총길이(mm)"],
            [
                ["기준(무패턴·무스냅·무회랑)", "185 / 208", "23.6 s", "1,474,000"],
                ["+ 학습면 투영(L2a)", "187 / 208", "6.4 s", "1,470,000"],
                ["+ 접근불가 스냅(전처리)", "194 / 208", "6.7 s", "1,442,800"],
                ["+ 기존설계 회랑(L2b, 옵트인)", "198 / 208", "46 s", "1,458,300"],
            ],
        )),
        ("p", "L2a 의 가속(23.6s→6.4s)은 잘못된 면으로 나가 수백만 노드를 헛탐색하던 배관이 학습 면 투영으로 "
              "사라진 효과다. 전처리는 솔리드에 파묻힌 PoC 7건을 복구했다. L2b 회랑은 +4 성공·총길이↑(설계 정합)"
              "을 내지만 w_corridor 페널티로 ~7× 느려 기본 OFF(옵트인)다. 남은 14→10 실패는 expanded>0(경로 없음 "
              "= 혼잡/막힘)으로 rip-up/CBS 영역이며 패턴 범위 밖이다. ctest golden/capi/implicit/attract/corridor "
              "5/5 불변. 상세 계획·설계는 docs/routing3d_pattern_learning_plan.{docx,pdf}."),
    ]


def ch_conclusion():
    return [
        ("h1", "7. 전체 결과 요약 및 향후 과제"),
        ("h2", "7.1 달성 요약"),
        ("b", [
            ...
            "기존설계 패턴 학습(P3j, 6장): 사람이 그린 기존배관 스텁을 pgvector 로 학습해 자동라우팅에 적용 "
            "— 학습면 PoC 투영·접근불가 PoC 스냅·기존설계 회랑(L2b). project6 c100 에서 185→198/208(+13)·"
            "최대 3.7× 가속(헛탐색 제거).",
        ]),
        ("h2", "7.2 성능 목표 대비 현황"),
        ("table", (
        ...
        ("p", "본 보고서는 2026-06-02 시점의 구현을 기준으로 한다(P3j 기존설계 패턴 학습 포함). "
              "최신 상태는 코드와 git 이력이 정답이다."),
    ]
...
def main():
    ...
    add_cover(doc)
    doc.add_page_break()
    for chapter in (ch_intro, ch_overview, ch_phase1, ch_phase2, ch_phase3, ch_csharp, ch_pattern,
                    ch_conclusion):
        emit(doc, chapter())
