...
def build():
    blocks = [
        ...
        ("n", [
            ...
            "방향 런 압축·지터 흡수(_dir_runs·_merge_short_runs): 각 세그먼트를 직교 축으로 스냅(axis_snap)해 "
            "방향 런으로 압축하고, STUB_MIN_DIR_RUN_MM(250mm) 미만 런은 설계 지터로 보아 인접 런에 흡수한다. "
            "이로써 수직 하강 중 미세 옵셋 지터가 가짜 꺾임으로 잡혀 진짜 엘보를 가리는 일을 막는다.",
            "엘보 포함(_walk_stub): 스텁은 ‘수직배관까지’가 아니라 ‘수직 + 첫 엘보(수직축→수평축 전환) + 짧은 "
            "수평 리드인(STUB_LEADIN_MM=800mm)’까지 본다. 첫 방향 런의 축을 수직축으로 보고, 축이 다른 첫 런을 "
            "엘보로 잡아 dir_seq 에 포함한다(꺾임 수 = len(dir_seq) − 1). 엘보가 없으면 STUB_MAX_MM(4000mm)·"
            "STUB_MAX_BENDS(3) 한도까지. 예) 장비 스텁 −z(수직)→+x(엘보, 랙으로 꺾임) ⇒ dir_seq = ‘−z,+x’.",
        ...
        ("table", [["구간", "차원", "내용"], [
            ...
            ["1차 방향 1-hot", "6", "dir_seq[0] (수직배관 진행 축)"],
            ["2차 방향 1-hot", "6", "dir_seq[1] = 엘보 방향(수직→수평 전환). 엘보 없으면 전부 0"],
