...
def build():
    blocks = [
        ...
        ("h2", "13.4 L2b(회랑 소프트 바이어스) — 구현 완료"),
        ("p", "corridor 키를 64비트로 확장(cost.hpp/astar.hpp/multi_route.hpp — ImplicitOccupancy 대형 "
               "격자 안전, 골든 불변)하고, C ABI r3d_set_corridor_cells 를 신설해 학습 회랑 셀을 엔진에 "
               "주입한다. route_multi 가 w_corridor>0 일 때 이 셀들을 회랑 시드로 삼는다. C# 뷰어는 각 "
               "작업에 매칭되는 기존 설계배관(TB_ROUTE_PATH) 폴리라인을 셀로 복셀화(±2)해 회랑으로 주입하고 "
               "w_corridor(=½칸 비용)로 회랑 밖을 가산 → 새 경로가 사람 설계를 부드럽게 따라간다(충돌은 여전히 "
               "회피). 경로 모양을 바꾸므로 기본 OFF(옵트인, UseDesignCorridor 토글)."),
        ("table", (
            ["구성", "성공", "시간", "총길이(mm)"],
            [
                ["회랑 OFF (기준 = 패턴+스냅)", "194 / 208", "6.5 s", "1,442,800"],
                ["회랑 ON (기존설계 추종)", "198 / 208", "46 s", "1,458,300"],
            ],
        )),
        ("p", "회랑 ON: +4 성공(회랑 가이드가 자유 최단탐색이 못 찾던 채널로 유도) · 총길이 +15,500"
               "(사람 설계를 따라 약간 길어짐 = 설계 정합) · 단 ~7× 느림(w_corridor 페널티로 일부 배관 "
               "탐색량 급증, 2건 탐색상한 근접). 골든/capi/implicit/attract ctest 5/5 불변(회랑은 "
               "w_corridor>0 일 때만 동작). 향후 과제: w_corridor 튜닝·회랑 폭 적응으로 속도 개선."),
