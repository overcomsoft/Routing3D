...
# Step 1.4 다중 배관 순차 라우팅(충돌 없이) — 전체 배관, 우선순위 longest
..\.venv\Scripts\python.exe -m routing3d_py.scene --project 6 --multi --priority longest --cell-mm 100 --screenshot out/multi.png
```
...
# 충돌 없이 순차 라우팅(깔린 경로를 점유로 추가) — 성공률/유틸리티별 지표
